...
type Communicator interface {
	...
	Execute(machine Machine, command []string, options ...interface{}) (status int32, err error)
	Init(machine Machine) error
	Match(machine Machine) (isMatch bool, err error)
	PrivilegedExecute(machine Machine, command []string, options ...interface{}) (status int32, err error)
	Ready(machine Machine) (isReady bool, err error)
	Reset(machine Machine) error
	Test(machine Machine, command []string, options ...interface{}) (valid bool, err error)
